...
class Algorithm:
    ...
    def accept(self, observation):
        ...
        self.indicator = self.indicator.append(
            pd.Series(observation), ignore_index=True
        )

    def balance(self):
        # Compute the balance remaining
        self.indicator["balance_change"] = (
            -1 * self.indicator["action"] * self.indicator["o"]
        )
        self.indicator["stock"] = self.indicator["action"].cumsum()
        self.indicator["balance"] = (
            self.capital + self.indicator["balance_change"].cumsum()
        )
    ...
    def performance(self):
        ...
        #         self.indicator['beta'] =
        self.indicator["sharpe"] = (
            self.indicator["return"].mean() / self.indicator["return"].std()
        )
        self.indicator["annual_sharpe"] = 252 ** 0.5 * self.indicator["sharpe"]
        self.indicator["sortino"] = (
            self.indicator["return"].mean()
    ...
    def statistics(self, k=100):
        return self.indicator[self.data_col + self.performance_col].iloc[-1].to_dict()
...
class MACDStrategy:
    ...
    def action(self, indicator):
        ...
        indicator["action"] = (
            np.sign(indicator["trend_macd_diff"])
            - np.sign(indicator["trend_macd_diff_prev"])
        ) / 2
...
class MFIStrategy:
    ...
    def action(self, indicator):
        ...
        indicator["sell"] = (
            ...
            & (indicator["volume_mfi"] < self.parameters["mfi80"])
        ).astype(int)
        indicator["buy"] = (
            (indicator["volume_mfi_prev"] < self.parameters["mfi20"])
            & (indicator["volume_mfi"] >= self.parameters["mfi20"])
        ).astype(int)
...
class RSIStrategy:
    ...
    def action(self, indicator):
        ...
        indicator["buy"] = (
            ...
            & (indicator["momentum_rsi"] < self.parameters["rsi30"])
        ).astype(int)
        indicator["action"] = indicator["buy"] - indicator["sell"]
